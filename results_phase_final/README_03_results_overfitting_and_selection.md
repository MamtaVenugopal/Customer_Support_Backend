...
![Leaderboard](results_phase_final/leaderboard.png)
![Source vs Target](results_phase_final/source_vs_target.png)
![K-fold Variance](results_phase_final/kfold_variance.png)

Winner (`mixed_mobilenet`) train-vs-val:

![Winner K-fold Overview](results_phase_final/checkpoints/mixed_mobilenet_kfold_overview.png)

Trained UNet (`mixed_unet`) train-vs-val:

![UNet K-fold Overview](results_phase_final/checkpoints/mixed_unet_kfold_overview.png)
...
Because source/target domain labels are hidden in eval-test:

- domain-wise confusion analysis is not possible without domain metadata.
- if anomaly/normal ground truth exists locally, a standard confusion matrix can still be computed.
- for official DCASE eval-test (labels hidden), only score outputs are available offline and confusion matrix requires official scoring labels.
